...
///
/// Unlike the `AnyDynamicType` conversion path, `decode` returns the concrete `Self` directly,
/// so the macro-synthesized bindings that call it never box the value as `Any` or cast it back
/// with `as!`. It is the JS → native half of `JavaScriptCodable`.
///
/// The conversion runs on the JavaScript thread; conformers are called under `@JavaScriptActor`.
/// The `appContext` and `runtime` are passed explicitly: `runtime` is `borrowing` because the
/// converters only read it, while `appContext` is a plain reference because some conversions
/// (e.g. shared objects) store or escape it.
public protocol JavaScriptDecodable {
  ...
  /// `Promise`).
  @JavaScriptActor
  static func decode(_ value: JavaScriptValue, appContext: AppContext, runtime: borrowing JavaScriptRuntime) throws -> Self
  ...
  @JavaScriptActor
  static func decode(_ value: borrowing JavaScriptUnownedValue, appContext: AppContext, runtime: borrowing JavaScriptRuntime) throws -> Self
}
...
extension JavaScriptDecodable {
  ...
  @JavaScriptActor
  @inlinable
  public static func decode(_ value: borrowing JavaScriptUnownedValue, appContext: AppContext, runtime: borrowing JavaScriptRuntime) throws -> Self {
